...
const News = (props) => {
    ...
    useEffect(() => {
        document.title= `${captalizefun(props.category)} - NewsMonkey`;
        updateNews();
    ...
    const fetchMoreData = async () => {        
        const url = `https://newsapi.org/v2/top-headlines?apiKey=${props.apiKey}&country=${props.country}&category=${props.category}&pageSize=${props.pageSize}&page=${page+1}`; 
        setPage(page+1)        
        let data = await fetch(url);
        let parsedData = await data.json();
    ...
    return (
        <>
       <h2 className='text-center'style={{margin:"35px 0px",marginTop:"90px"}}>NewsMonkey - Top Headlines from {captalizefun(props.category)}</h2> 
        {loading && <Spinner />}       
        <InfiniteScroll
